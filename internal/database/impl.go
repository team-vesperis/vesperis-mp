...
func new(ctx context.Context, r *redis.Client, l *zap.SugaredLogger, p *pgxpool.Pool) *database {
	lm := listenManager{
...
func (d *database) Get(key string) (any, error) {
	...
	*/
	...
	if err := cmd.Err(); err == nil {
		...
		d.l.Errorw("redis get error", "key", key, "error", err)
	}
	...
	if err != nil {
		d.l.Errorw("postgres query error", "key", key, "error", err)
		return "", err
	}
	defer r.Close()
	for r.Next() {
		scanErr := r.Scan(&val)
		if scanErr != nil {
			d.l.Errorw("postgres scan error", "key", key, "error", scanErr)
			return "", scanErr
		}

		if r.Err() != nil {
			d.l.Errorw("postgres rows error", "key", key, "error", r.Err())
			return "", r.Err()
		}
	}
	...
	if val != "" {
		...
		if err != nil {
			d.l.Warnw("redis set error", "key", key, "error", err)
		}
	}
...
func (d *database) Set(key string, val any) error {
	...
	if cmd.Err() != nil {
		d.l.Errorw("redis set error", "key", key, "value", val, "error", cmd.Err())
	}
	...
	if err != nil {
		d.l.Errorw("postgres upsert error", "key", key, "value", val, "error", err)
		return err
	}
...
func (d *database) SetPlayerData(playerId string, data map[string]any) error {
	...
	if err != nil {
		d.l.Errorw("marshal error", "data", data, "error", err)
		return err
	}
	...
	if err != nil {
		d.l.Errorw("postgres upsert error", "playerId", playerId, "error", err)
		return err
	}
...
func (d *database) GetPlayerData(playerId string) (map[string]any, error) {
	...
	if err != nil {
		...
		d.l.Errorw("postgres read error", "playerId", playerId, "error", err)
		return nil, err
	}

	if err := json.Unmarshal(jsonData, &playerData); err != nil {
		d.l.Errorw("json unmarshal error", "playerId", playerId, "error", err)
		return nil, err
	}
...
func (d *database) SetPlayerDataField(playerId, field string, val any) error {
	...
	if err != nil {
		d.l.Errorw("postgres update error", "playerId", playerId, "field", field, "error", err)
		return err
	}
	return nil
}
...
func (d *database) GetPlayerDataField(playerId, field string) (any, error) {
	...
	if err != nil {
		...
		d.l.Errorw("postgres read error", "playerId", playerId, "field", field, "error", err)
		return nil, err
	}

	var result any
	if err := json.Unmarshal(fieldData, &result); err != nil {
		d.l.Errorw("json unmarshal error", "playerId", playerId, "field", field, "error", err)
		return nil, err
	}
...
func (d *database) Publish(channel string, message any) error {
	...
	if cmd.Err() != nil {
		d.l.Errorw("redis publish error", "channel", channel, "message", message, "error", cmd.Err())
	}
...
func (d *database) CreateListener(channel string, handler func(msg *redis.Message)) {
	...
	if _, exists := d.lm.m[channel]; exists {
		d.l.Warn("Listener already exists for channel: ", channel)
		return
	}
	...
	go func() {
		for {
			...
			if err != nil {
				...
				d.l.Error("Error receiving message on channel ", channel, ": ", err)
				continue
			}
			d.lm.wg.Add(1)
			handler(msg)
...
func (d *database) DeleteListener(channel string) error {
	...
	err := pubsub.Close()
	if err != nil {
		d.l.Errorw("redis closing pubsub error", "channel", channel, "error", err)
		return err
	}
	return nil
}
...
func (d *database) Close() {
	...
	err := d.r.Close()
	if err != nil {
		d.l.Errorw("redis close error", "error", err)
	}

	err = d.m.Close()
	if err != nil {
		d.l.Errorw("mysql close error", "error", err)
	}
